import React, { useContext, useEffect, useState } from 'react';
import { View, Text, StyleSheet, Image, Dimensions, ActivityIndicator, Alert, TextInput, Pressable } from 'react-native';
import TwokLoader from './ViewModel/TwokLoader';
import ComunicationController from './Model/ComunicationController';
import { UserContext } from './Model/UserContext';
import { Ionicons } from '@expo/vector-icons';
import CommunicationController from './Model/ComunicationController';

const UserProfile = (props) => {

  const SID = useContext(UserContext)

  const [profilo, setProfilo] = useState({ name: "caricando nome" })
  const [pic, setPic] = useState("")
  const [seguito, setSeguito] = useState()
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    getProfilo()
  }, [])

  const getProfilo = async () => {
    let p = await ComunicationController.serverReq("getPicture", { sid: SID, uid: props.dati })
    console.log(p)
    setProfilo(p)
    if (p.picture != null) setPic(p.picture)
    let f = await ComunicationController.serverReq("isFollowed", { sid: SID, uid: props.dati })
    console.log(f)
    setSeguito(f.followed)
    setLoading(false)
  }

  const createFollowButton = () => {
    if (seguito) {
      return (
        <Pressable style={styles.unfollow} onPress={() => { console.log("Hai deciso di smettere di seguire utente " + props.dati), setSeguito(false), followUnfollow('u') }}>
          <Text style={{ fontSize: 14, fontWeight: "400", color: 'black', }}>
            <Ionicons size={18} name={"checkmark-outline"} /> Unfollow</Text>
        </Pressable>
      )
    } else {
      return (
        <Pressable style={styles.follow} onPress={() => { console.log("Hai deciso di seguire utente " + props.dati), setSeguito(true), followUnfollow('f') }}>
          <Text style={{ fontSize: 14, fontWeight: "400", color: 'black', }}>
            <Ionicons size={18} name={"checkmark-outline"} /> Follow</Text>
        </Pressable>
      )
    }
  }

  const followUnfollow = async (param) => {
    let r;
    switch (param) {
      case 'f':
        r = await CommunicationController.serverReq("follow", { sid: SID, uid: profilo.uid })
        if (r != null) Alert.alert("Hai iniziato a seguire utente " + profilo.name)
        break
      case 'u':
        r = await CommunicationController.serverReq("unfollow", { sid: SID, uid: profilo.uid })
        if (r != null) Alert.alert("Hai smesso di seguire utente " + profilo.name)
        break
      default:
        Alert.alert("Errore app, riavviare")
        break
    }
  }

  if (!loading) {
    return (
      <View style={{ flex: 1 }}>
        <View style={{ backgroundColor: `#fffafa` }}>
          <Text style={{ padding: 2 }}>
            <View style={{ padding: 6, paddingBottom: 18 }}>
              <Pressable style={styles.torna} onPress={() => props.onBack()}>
                <Text style={{
                  fontSize: 14, fontWeight: "350",
                  color: 'white',
                }}><Ionicons size={18} name={"return-up-back-outline"} />Torna</Text>
              </Pressable>
            </View>
            <View style={{ padding: 10 }}>
              <Text >
                <View style={{ paddingRight: 6 }}>
                  <Image style={styles.logo} source={{ uri: 'data:image/png;base64,' + pic }} />
                </View>
                <View style={{ paddingBottom: 14 }}>
                  <Text style={{ fontSize: 20, fontWeight: "600", paddingRight: 12 }}>{profilo.name}
                  </Text>
                </View>
              </Text>
            </View>
            <View style={{ paddingBottom: 18, alignItems: 'flex-end', justifyContent: 'flex-end' }}>
              {createFollowButton()}
            </View>
          </Text>
        </View>

        <View style={{ height: Dimensions.get('window').height / 1.4 }}>
          <TwokLoader MINE={false} profilo={profilo} />
        </View>
      </View>
    )
  }
}

const styles = StyleSheet.create({
  logo: {
    borderRadius: 100,
    width: 60,
    height: 60,
  },
  torna: {
    borderRadius: 8,
    padding: 6,
    height: "9%",
    width: '20%',
    backgroundColor: '#ff5c69',
    justifyContent: 'center',
    alignItems: 'center',
    elevation: 10,
  },
  follow: {
    borderRadius: 8,
    padding: 6,
    height: "9%",
    width: '20%',
    backgroundColor: '#c7ffab',
    elevation: 10,
  },
  unfollow: {
    borderRadius: 8,
    padding: 6,
    height: "9%",
    width: '20%',
    backgroundColor: '#ff9387',
    elevation: 10,
  }
});

export default UserProfile;