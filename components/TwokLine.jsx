import React, { Component, useState } from 'react';
import { View, Button, Text, StyleSheet, Br, Image, Dimensions, Pressable } from 'react-native';
import { Ionicons } from '@expo/vector-icons';


export default function TwokLine(props) {
  const profile = props.p
  const halign = props.dati.halign
  const valign = props.dati.valign
  const fontType = props.dati.fonttype
  const fontSize = props.dati.fontsize
  const pic = props.dati.picture

  const manageAlign = (align) => {
    switch (align) {
      case 0:
        return 'flex-start'
      case 1:
        return 'center'
      case 2:
        return 'flex-end'
    }
  }

  const manageFontType = (ft) => {
    switch (ft) {
      case 0:
        return 'System'
      case 1:
        return 'monospace'
      case 2:
        return 'serif'
    }
  }

  const manageSize = (fs) => {
    switch (fs) {
      case 0:
        return 12
      case 1:
        return 22
      case 2:
        return 32
    }
  }

  const checkPic = () => {
    if (pic == null) return ""
    else return pic
  }

  return (profile ?
    <View>
      <View style={{
        width: "100%",
        height: Dimensions.get('window').height / 1.44,
        backgroundColor: "#" + props.dati.bgcol,
        alignItems: manageAlign(halign),
        justifyContent: manageAlign(valign),
      }}>
        <Text style={{
          fontSize: manageSize(fontSize),
          fontWeight: "700",
          color: "#" + props.dati.fontcol,
          fontFamily: manageFontType(fontType)
        }}>
          {props.dati.text+" "}
          <Pressable style={styles.pressable} onPress={() => props.onMap()}>
            <Ionicons size={18} name={"location-sharp"} />
          </Pressable>
        </Text>
      </View>
    </View>
    : //home
    <View>
      <Pressable onLongPress={() => props.onBigMap()}>
      <View style={{
        width: "100%",
        height: Dimensions.get('window').height / 1.23,
        backgroundColor: "#" + props.dati.bgcol,
        alignItems: manageAlign(halign),
        justifyContent: manageAlign(valign),
      }}>
        <View ><Text ><View><Image style={styles.logo} source={{ uri: 'data:image/png;base64,' + checkPic() }} /></View>
        <View>
          <Text onPress={() => props.onProfile()} style={{
          fontSize: 16,
          paddingBottom: 8
        }}>{" " + props.dati.name}</Text></View></Text></View>
        <Text style={{
          fontSize: manageSize(fontSize),
          fontWeight: "700",
          color: "#" + props.dati.fontcol,
          fontFamily: manageFontType(fontType)
        }}>
          {props.dati.text}
          <Pressable style={styles.pressable} onPress={() => props.onMap()}>
            <Ionicons size={18} name={"location-sharp"} />
          </Pressable>
        </Text>
      </View>
      </Pressable>
    </View>)
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#fff',
    alignItems: 'center',
    justifyContent: 'center',
  },
  pressable: {
    borderRadius: 8,
    padding: 1,
    height: "4%",
    width: '10%',
    backgroundColor: 'white',
    justifyContent: 'center',
    alignItems: 'center',
    elevation: 10,
  },
  logo: {
    width: 40,
    borderRadius: 40,
    height: 40,
  },
});