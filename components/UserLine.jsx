import React, { useContext, useEffect, useState } from 'react';
import { View, Button, Text, StyleSheet, Br, Image, Dimensions, Pressable } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { UserContext } from './Model/UserContext';
import ComunicationController from './Model/ComunicationController';


export default function UserLine(props) {

  const SID = useContext(UserContext)
  const PLACEHOLDER_IMAGE = ""

  const userID = props.dati.uid
  const name = props.dati.name
  const [pic, setPic] = useState(null)

  useEffect(() => {
    getPicture()
  }, [])

  const getPicture = async () => {
    let params = { sid: SID, uid: userID }
    let r = await ComunicationController.serverReq("getPicture", params)
    if (r != null) setPic(r.picture)
  }

  const checkPic = () => {
    if (pic == null) return PLACEHOLDER_IMAGE
    else return pic
  }

  return (
    <View>
      <Text style={{padding:10}} >
        <View>
          <Image style={styles.logo} source={{ uri: 'data:image/png;base64,' + checkPic() }} />
        </View>
        <View>
          <Text style={{ paddingBottom:10,
            fontSize: 16,
          }}>{" " + name}
          </Text>
        </View>
      </Text>
    </View>
  )
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#fff',
    alignItems: 'center',
    justifyContent: 'center',
  },
  pressable: {
    borderRadius: 8,
    padding: 1,
    height: "4%",
    width: '10%',
    backgroundColor: 'white',
    justifyContent: 'center',
    alignItems: 'center',
    elevation: 10,
  },
  logo: {
    width: 60,
    borderRadius: 60,
    height: 60,
  },
});